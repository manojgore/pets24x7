...
const GalleryUploader = ({ serviceFormData, setServiceFormData }) => {
  const [error, setError] = useState("");

  const handleFileUpload = (event) => {
    const file = event.target.files[0];
    const maxSize = 1800; // Maximum allowed width and height in pixels
  ...
  return (
    <div className="row x-gap-20 y-gap-20 pt-15">
      ...
      <div className="col-auto">
        <div className="w-200">
          <label htmlFor="uploadService" className="d-flex ratio ratio-2:1">
      ...
      {serviceFormData?.service_img?.length > 0 && (
        <div className="col-auto">
          <div className="d-flex ratio ratio-2:1 w-200">
            <img
              src={serviceFormData.service_img[0]}
              alt="Uploaded"
