...
const GalleryUploader = ({vendorData, setVendorData}) => {
          ...
          if (img.width > maxSize || img.height > maxSize) {
            ...
            if (newImages.length === fileList.length) {
              setVendorData((prevState) => ({
                ...prevState,
                portfolio: [...vendorData?.portfolio, ...newImages],
              }));
              setError("");
            }
          }
  ...
  const handleRemoveImage = (index) => {
    const newImages = [...vendorData?.portfolio];
  ...
  return (
    <div className="row x-gap-20 y-gap-20 pt-15">
      ...
      {vendorData?.portfolio.length !== 0 && vendorData?.portfolio.map((image, index) => (
